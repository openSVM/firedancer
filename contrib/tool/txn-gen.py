...
ACCT_ACCESS_DIST_REGULAR = 0
ACCT_ACCESS_DIST_POWER     = 1

NANO_TOKEN_ID = Pubkey.from_string("GjyKyRCSygSaszrjJFu43jkAshFc1sWs45HqKEDXhvwx")
NOOP_ID = Pubkey.from_string("NoopToken1111111111111111111111111111111111")
...
nano_mint = Keypair.from_seed_and_derivation_path(top_seed, f"m/44'/45'/30'/99996'")
...
def get_account_info(rpc: str, acc: str):
    data = f"{{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"getAccountInfo\",\"params\":[\"{acc}\",{{\"commitment\":\"processed\",\"encoding\":\"base64\"}}]}}"
    resp = requests.post(rpc, data=data, headers={"Content-Type": "application/json"})
    if resp.status_code != 200:
        return None
        ...
        return base64.b64decode(resp.json()["result"]["value"]["data"][0])
...
def fund_config_account2( funder, lamports, recent_blockhash, range ):
    tx = Transaction(recent_blockhash, None, funder.pubkey(),[set_compute_unit_price(3), set_compute_unit_limit(300_000)])
    zero = 0
...
def fund_nano_mint_account2( funder, lamports, recent_blockhash, range ):
  tx = Transaction(recent_blockhash, None, funder.pubkey(),[set_compute_unit_price(3), set_compute_unit_limit(300_000)])
...
def fund_token_account(funder, lamports, recent_blockhash, is_print, range ):
    tx = Transaction(recent_blockhash, None, funder.pubkey(),[set_compute_unit_price(3), set_compute_unit_limit(200_000)])
...
def get_balance_sufficient(lamports, rpc: str, txn_type, acc):
    if txn_type == TXN_TYPE_NANO_TOKEN_TRANSFER:
        zero   = 0
        seeds1 = zero.to_bytes(8, byteorder="little", signed=False) # mint
        seeds2 = bytes(acc.pubkey()) # owner
        seeds  = [seeds2, seeds1]
        nano_ata, nano_ata_bump = Pubkey.find_program_address( seeds=seeds, program_id=NANO_TOKEN_ID )
        data = get_account_info(rpc, nano_ata)
        if not data:
          return None
...
def create_accounts(funder, rpc, num_accs, lamports, seed, sock,  tpus, txn_type):
    mint_data = get_account_info(rpc, nano_mint.pubkey())
    ...
    print(get_balance( rpc, NANO_TOKEN_ID ))
    print(get_balance( rpc, config_acc.pubkey() ))

    get_account_info(rpc, config_acc.pubkey())
    get_account_info(rpc, nano_mint.pubkey())
    if txn_type == TXN_TYPE_NANO_TOKEN_TRANSFER:
        while not get_account_info(rpc, config_acc.pubkey()):
          recent_blockhash = get_recent_blockhash(rpc)
          txs = [fund_config_account( funder, lamports, recent_blockhash, 1 )]
          send_round_of_txs(txs, sock, tpus)
          time.sleep(3)
        print("CONFIG CREATED ", get_account_info(rpc, config_acc.pubkey()) )
        config_data = get_account_info(rpc, config_acc.pubkey())
        print("CONFIG DATA", config_data)
        while config_data[0] != 1:
          recent_blockhash = get_recent_blockhash(rpc)
          txs = pqdm([i for i in range(1)], partial(fund_config_account2, funder, lamports, recent_blockhash), desc="fund config", n_jobs=32)
          send_round_of_txs(txs, sock, tpus)
          time.sleep(3)
          config_data = get_account_info(rpc, config_acc.pubkey())
        print("CONFIG INITED")

        while not get_account_info(rpc, nano_mint.pubkey()):
          recent_blockhash = get_recent_blockhash(rpc)
          txs = pqdm([i for i in range(1)], partial(fund_nano_mint_account2, funder, lamports, recent_blockhash), desc="fund mint", n_jobs=32)
          print(txs)
          send_round_of_txs(txs, sock, tpus)
          time.sleep(3)
        print("MINT CREATED")
        ...
        while mint_data[0] == 0:
          ...
          recent_blockhash = get_recent_blockhash(rpc)
          txs = [fund_nano_mint_account( funder, lamports, recent_blockhash, 1 )]
          send_round_of_txs(txs, sock, tpus)
          time.sleep(3)
          mint_data = get_account_info(rpc, nano_mint.pubkey())
    ...
    while len(remaining_accs) > 0:
        ...
        done_accs = pqdm(remaining_accs, partial(get_balance_sufficient, lamports/10, rpc, txn_type), desc="check bal", n_jobs=32)

        for acc in done_accs:
            if acc is not None:
                remaining_accs.remove(acc)
        print(len(remaining_accs))
        print(get_balance(rpc, funder.pubkey()))
...
def gen_tx_nano_token_transfer(recent_blockhash, src_key, src_acc, src_nano_ata, dst_nano_ata, cu_price):
  tx = Transaction(recent_blockhash, None, src_acc, [set_compute_unit_price(cu_price), set_compute_unit_limit(300+48)] )
  ...
  # data1 = tag.to_bytes(8, byteorder="little", signed=False)
  # data2 = amt.to_bytes(8, byteorder="little", signed=False)
  data2 = b'\x01\x00\x00\x00\x00\x00\x00\x00'
  data  =  data2
  keys = [
...
def send_txs(rpc: str, tpus: List[str], keys: List[Keypair], tx_idx, mult, idx, stop_event, rbh, txn_type: int, acct_access_distr: int):
  ...
  for acc in accs:
    zero   = 0
    seeds1 = zero.to_bytes(8, byteorder="little", signed=False) # mint
    seeds2 = bytes(acc) # owner
    seeds  = [seeds2, seeds1]
    nano_ata, _ = Pubkey.find_program_address( seeds=seeds, program_id=NANO_TOKEN_ID )
    nano_atas.append(nano_ata)
  ...
  niter = len(keys)
  if acct_access_distr == ACCT_ACCESS_DIST_REGULAR and txn_type == TXN_TYPE_NANO_TOKEN_TRANSFER:
    niter = len(keys)//2
  alpha = 10.0
  while not stop_event.is_set():
    ...
    for k in range(niter):
      if acct_access_distr == ACCT_ACCESS_DIST_REGULAR:
        i = k
        j = (i + (len(keys) // 2)) % len(keys)
      elif acct_access_distr == ACCT_ACCESS_DIST_POWER:
        i = np.floor(np.random.power(alpha)*len(keys)).astype(int)
        j = np.floor(np.random.power(alpha)*len(keys)).astype(int)
        if i == j:
          j += 1
          if j == len(keys):
            continue
      if txn_type == TXN_TYPE_EMPTY:
        ...
        tx = gen_tx_nano_token_transfer(recent_blockhash, keys[i], accs[i], nano_atas[i], nano_atas[j], cu_price)
      message = bytes(tx.to_solders())
      for tpu in tpus:
        sock.sendto(message, tpu)
...
def main():
  ...
  if args.acct_access_distr == "regular":
    acct_access_distr = ACCT_ACCESS_DIST_REGULAR
  elif args.acct_access_distr == "power":
    acct_access_distr = ACCT_ACCESS_DIST_POWER
  else:
    print("unknown acct access distribution type")
    exit(1)

  accs = create_accounts(funder, args.rpc, args.nkeys, 10_00_000_000, seed, sock, tpus, txn_type)
