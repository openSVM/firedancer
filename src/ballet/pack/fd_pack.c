...
struct fd_pack_private {
  ulong      pack_depth;
  ulong      bundle_depth; /* if 0, bundles are disabled */
  ulong      bank_tile_cnt;
...
fd_pack_footprint( ulong                    pack_depth,
  ...
  ulong l;
  ulong total_depth        = pack_depth + bundle_depth*FD_PACK_MAX_TXN_PER_BUNDLE;
  ulong extra_depth        = fd_ulong_if( bundle_depth>0UL, FD_PACK_MAX_TXN_PER_BUNDLE, 1UL ); /* space for use between init and fini */
  ulong max_acct_in_treap  = total_depth * FD_TXN_ACCT_ADDR_MAX;
  ...
  ulong bundle_temp_accts  = fd_ulong_if( bundle_depth>0UL, FD_PACK_MAX_TXN_PER_BUNDLE*FD_TXN_ACCT_ADDR_MAX, 1UL );
  ...
  int lg_depth       = fd_ulong_find_msb( fd_ulong_pow2_up( 2UL*total_depth                               ) );
  ...
  l = FD_LAYOUT_APPEND( l, trp_pool_align (),   trp_pool_footprint ( total_depth+extra_depth  ) ); /* pool           */
  l = FD_LAYOUT_APPEND( l, penalty_map_align(), penalty_map_footprint( lg_penalty_trp         ) ); /* penalty_treaps */
  l = FD_LAYOUT_APPEND( l, expq_align     (),   expq_footprint     ( total_depth              ) ); /* expiration prq */
...
fd_pack_new( void                   * mem,
  ...
  ulong total_depth        = pack_depth + bundle_depth*FD_PACK_MAX_TXN_PER_BUNDLE;
  ulong extra_depth        = fd_ulong_if( bundle_depth>0UL, FD_PACK_MAX_TXN_PER_BUNDLE, 1UL );
  ulong max_acct_in_treap  = total_depth * FD_TXN_ACCT_ADDR_MAX;
  ulong max_acct_in_flight = bank_tile_cnt * (FD_TXN_ACCT_ADDR_MAX * limits->max_txn_per_microblock + 1UL);
  ulong max_txn_in_flight  = bank_tile_cnt * limits->max_txn_per_microblock;
  ...
  /* The pool has one extra element that is used between insert_init and
     cancel/fini. */
  void * _pool        = FD_SCRATCH_ALLOC_APPEND( l,  trp_pool_align(),    trp_pool_footprint ( total_depth+extra_depth) );
  void * _penalty_map = FD_SCRATCH_ALLOC_APPEND( l,  penalty_map_align(), penalty_map_footprint( lg_penalty_trp       ) );
  void * _expq        = FD_SCRATCH_ALLOC_APPEND( l,  expq_align(),        expq_footprint     ( total_depth            ) );
  ...
  pack->bundle_depth                = bundle_depth;
  ...
  trp_pool_new(  _pool,        total_depth+extra_depth );

  fd_pack_ord_txn_t * pool = trp_pool_join( _pool );
  treap_seed( pool, total_depth+extra_depth, fd_rng_ulong( rng ) );
  for( ulong i=0UL; i<total_depth+extra_depth; i++ ) pool[i].root = FD_ORD_TXN_ROOT_FREE;
  ...
  treap_new( (void*)pack->pending_bundles, bundle_depth*FD_PACK_MAX_TXN_PER_BUNDLE );
...
fd_pack_join( void * mem ) {
  ...
  ulong total_depth            = pack->pack_depth + pack->bundle_depth*FD_PACK_MAX_TXN_PER_BUNDLE;
  ulong extra_depth            = fd_ulong_if( pack->bundle_depth>0UL, FD_PACK_MAX_TXN_PER_BUNDLE, 1UL );
  ...
  ulong written_list_max   = fd_ulong_min( max_w_per_block>>1, DEFAULT_WRITTEN_LIST_MAX );
  ulong bundle_temp_accts  = fd_ulong_if( pack->bundle_depth>0UL, FD_PACK_MAX_TXN_PER_BUNDLE*FD_TXN_ACCT_ADDR_MAX, 1UL );
  ...
  pack->pool          = trp_pool_join(   FD_SCRATCH_ALLOC_APPEND( l, trp_pool_align(),   trp_pool_footprint   ( total_depth+extra_depth ) ) );
  pack->penalty_treaps= penalty_map_join(FD_SCRATCH_ALLOC_APPEND( l, penalty_map_align(),penalty_map_footprint( lg_penalty_trp          ) ) );
  pack->expiration_q  = expq_join    (   FD_SCRATCH_ALLOC_APPEND( l, expq_align(),       expq_footprint       ( total_depth             ) ) );
...
fd_pack_insert_txn_fini( fd_pack_t  * pack,
  ...
  int validation_result = validate_transaction( pack, ord, txn, accts, alt_adj, pack->bundle_depth>0UL );
...
fd_pack_insert_bundle_fini( fd_pack_t          * pack,
  ...
  ulong pending_txn_cnt = treap_ele_cnt( pack->pending_bundles );
    /* We assume that the bundles are coming in a pre-prioritized order,
       so it doesn't make sense to drop an earlier bundle for this one.
       That means that really, the best thing to do is drop this one. */
  if( FD_UNLIKELY( pending_txn_cnt+txn_cnt>pack->bundle_depth*FD_PACK_MAX_TXN_PER_BUNDLE ) ) err = FD_PACK_INSERT_REJECT_PRIORITY;
  ...
  if( FD_UNLIKELY( !pending_txn_cnt ) ) {
...
insert_bundle_impl( fd_pack_t           * pack,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    fd_txn_t const * txn     = TXN(bundle[ i ]->txn);
    uchar    const * payload = bundle[ i ]->txn->payload;
    sig2txn_insert( pack->signature_map, fd_txn_get_signatures( txn, payload ) );
...
void
fd_pack_set_initializer_bundles_read( fd_pack_t * pack ) {
...
fd_pack_try_schedule_bundle( fd_pack_t  * pack,
  ...
  while( !(doesnt_fit | has_conflict) & !treap_rev_iter_done( _cur ) ) {
    ...
    for( fd_txn_acct_iter_t iter=fd_txn_acct_iter_init( txn, FD_TXN_ACCT_CAT_WRITABLE );
      ...
      in_bundle_temp->carried_cost += (uint)cur->compute_est; /* < 2^21 */
    ...
    for( fd_txn_acct_iter_t iter=fd_txn_acct_iter_init( txn, FD_TXN_ACCT_CAT_READONLY );
      ...
      fd_pack_addr_use_t * use = acct_uses_query( pack->acct_in_use,  *acct, NULL );
      if( FD_UNLIKELY( use->in_use_by & FD_PACK_IN_USE_WRITABLE ) ) {
        has_conflict = 1;
        FD_MCNT_INC( PACK, TRANSACTION_SCHEDULE_SLOW_PATH,  1UL );
        break;
      }
  ...
  for( ulong i=0UL; i<bundle_temp_inserted_cnt; i++ ) {
    ...
    use->in_use_by |= bank_tile_mask | fd_ulong_if( addr_use->carried_cost>0U, FD_PACK_IN_USE_WRITABLE, 0UL );
    use->in_use_by &= ~FD_PACK_IN_USE_BIT_CLEARED;
...
fd_pack_expire_before( fd_pack_t * pack,
  ...
  while( (expq_cnt( prq )>0UL) & (prq->expires_at<expire_before) ) {
    ...
    delete_transaction( pack, expired_sig, 0, 1 );
    deleted_cnt++;
  }
